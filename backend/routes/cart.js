...
const User = require("../models/userModel")
  ...
  router.post("/addToCart", userMiddleware, async (req, res) => {
    try {
        ...
        let quan = 0; // Initialize quantity accumulator
        for (const item of user.cart) {
            console.log("in add cart items ");
            // Perform operations on each item in the cart
        ...
        try {
            const result = await innovativeProd.aggregate([
                // Match documents where the title field matches the given product title
                { $match: { title: req.body.title } },
                // Group the matched documents and calculate the total quantity
                {
                    $group: {
                        _id: null, // Group by all documents (no specific field)
                        totalQuantity: { $sum: '$quantity' } // Calculate the sum of the quantity field
                    }
                }
            ]);
            ...
            console.log(totalQuantity, " - ", quan + 1);

            if (totalQuantity >= quan + 1) {
                user.cart.push(req.body);
                await user.save();
                res.status(201).json({ msg: "Item added to cart successfully" });
            } else {
                res.status(500).json({ msg: "Product Unavailable" });
            }
...
router.get("/getCartByEmail",userMiddleware, async (req, res) => {
    try {
        ...
        console.log(email+" email in get caret");
